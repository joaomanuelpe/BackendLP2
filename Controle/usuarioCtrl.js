...
export default class UsuarioCtrl {
    ...
    gravar(requisicao, resposta) {
        ...
        if (requisicao.method == 'POST' && requisicao.is("application/json")) {
            ...
            const tipo = new Tipo(requisicao.body.tipo.codigo);

            // Validação básica
            tipo.consultar((lista) => {
                if (lista.length > 0) {
                    if (nome && senha && email && dataNascimento && tipo) {
                        const usuario = new Usuario(0, nome, senha, email, dataNascimento, tipo.codigo > 0);
                        usuario.incluir()
                            .then(() => {
            ...
            tipo.consultar((lista) => {
                if (lista.length > 0) {
                    if (codigo > 0 && nome && senha && email && dataNascimento && tipo) {
                        const usuario = new Usuario(codigo, nome, senha, email, dataNascimento, tipo.codigo > 0);
                        usuario.alterar()
                            .then(() => {
                                resposta.status(200).json({
                                    "status": true,
